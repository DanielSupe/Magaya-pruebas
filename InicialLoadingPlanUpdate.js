...
function getItemsTable() {
    ...
    if (hasValue(items)) {
        var itemData = {};
        var containerizedItems = {};

        forEach(items, function (item) {
            var packageName = item.PackageName ? item.PackageName : "";
            var packageType = packageName.slice(0, 2);

            // Check if the item is containerized cargo
            if (['20', '40', '45'].includes(packageType)) {
                var HBOL = getHBOLorHAWB(item);
                if (!containerizedItems[HBOL]) {
                    containerizedItems[HBOL] = [];
                }
                // Store the containerized item data for the current HBOL
                containerizedItems[HBOL].push(getDataRowContainerized(item, true));
            } else {
                var HBOL = getHBOLorHAWB(item);
                if (!itemData[HBOL]) {
                    itemData[HBOL] = [];
                }
                // Store the item data for the current HBOL
                itemData[HBOL].push(getDataRow(item, true));
            }
        });

        // Sort HBOLs alphabetically for non-containerized cargo
        var sortedHBOLs = Object.keys(itemData).sort();

        // Construct the result string for non-containerized cargo in sorted order by HBOL
        for (var i = 0; i < sortedHBOLs.length; i++) {
            var HBOL = sortedHBOLs[i];
            // Add HBOL as a grouping line enclosed in double quotes
            result += '"' + HBOL + '"' + '\r\n';
            // Concatenate item data for the current HBOL
            result += itemData[HBOL].join('\r\n') + '\r\n';
            // Calculate subtotal for the current HBOL
            var subtotal = calculateSubtotal(itemData[HBOL]);
            // Add subtotal line
            result += '\t\tPieces Subtotal =>\t' + subtotal + '\t\t\t\t\r\n'; // Removed the "" in the first column
        }

        // Construct the result string for containerized cargo
        for (var HBOL in containerizedItems) {
            if (containerizedItems.hasOwnProperty(HBOL)) {
                // Add HBOL as a grouping line enclosed in double quotes
                result += '"' + HBOL + '"' + '\r\n';
                // Concatenate containerized item data for the current HBOL
                result += containerizedItems[HBOL].join('\r\n') + '\r\n';
                // Calculate subtotal for the current HBOL
                var subtotal = calculateSubtotal(containerizedItems[HBOL]);
                // Add subtotal line
                result += '\t\tPieces Subtotal =>\t' + subtotal + '\t\t\t\t\r\n'; // Removed the "" in the first column
            }
        }
